...
export const Grid: React.FC<GridProps> = ({ 
  ...
  return (
    <div className="w-full max-w-[500px] aspect-square border-2 border-[#f0ad25] rounded-xl overflow-hidden bg-white shadow-sm flex flex-col">
      {currentGrid.map((row, rIndex) => (
        <div key={rIndex} className={`flex flex-1 ${rIndex % 3 === 2 && rIndex !== 8 ? 'border-b-2 border-gray-300' : 'border-b border-gray-100'}`}>
            ...
            return (
              <div 
                ...
                className={`flex-1 flex items-center justify-center cursor-pointer relative text-xl md:text-2xl transition-colors
                  ${cIndex % 3 === 2 && cIndex !== 8 ? 'border-r-2 border-gray-300' : 'border-r border-gray-100'}
                  hover:bg-gray-50
                `}
              >
                {/* Highlight per casella vuota selezionata */}
                {isSelected && cellValue === 0 && (
                  <div className="absolute flex items-center justify-center w-8 h-8 md:w-11 md:h-11 rounded-full bg-[#f4f2eb] pointer-events-none"></div>
                )}
                ...
                `}>
                  <div className={`flex items-center justify-center w-8 h-8 md:w-11 md:h-11 rounded-full transition-colors duration-200
                    ${isError ? 'bg-red-500' : ((isSelected || isOccurrence) ? 'bg-[#f0ad25]' : (isInitial ? 'bg-[#f4f2eb]' : 'bg-transparent'))}
                  `}>
                    <span className={`transition-colors duration-200
                      ${isError ? 'text-white' : ((isSelected || isOccurrence) ? 'text-[#f4f2eb]' : (isInitial ? 'text-[#f0ad25]' : 'text-[#333333]'))} 
                  ...
                  {cellNotes.length > 0 && (
                    <div className="flex justify-center gap-[1px] md:gap-[3px] leading-none mb-[1px] md:mb-[2px]">
                      {cellNotes.slice(0, 2).map(n => (
                        <span key={n} className="text-[10px] md:text-xs font-semibold text-gray-600 w-[7px] md:w-2 text-center animate-pop-in">{n}</span>
                      ))}
                    </div>
                  )}
                  {/* Seconda riga (max 4) */}
