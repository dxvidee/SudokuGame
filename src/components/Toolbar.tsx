  ...
  return (
    <div className="w-full max-w-[500px] flex justify-between items-center px-2 mb-6">
      ...
      <div className="relative" ref={menuRef}>
        ...
        {showHintMenu && (
          <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-56 bg-white dark:bg-zinc-800 rounded-2xl shadow-xl border border-gray-100 dark:border-zinc-700 p-2 z-50 flex flex-col gap-1 animate-pop-in">
            <button 
              onClick={() => { onHint(); setShowHintMenu(false); }}
              className="flex items-center gap-3 w-full px-4 py-3 text-sm font-semibold text-gray-700 dark:text-zinc-200 hover:bg-[#f8f6f0] dark:hover:bg-zinc-700 hover:text-[#f0ad25] rounded-xl transition-colors text-left"
            >
              <Lightbulb size={18} />
      ...
      <button onClick={onNotesToggle} className="flex flex-col items-center justify-center group active:scale-90 transition-all" title="Appunti">
        <div className={`w-12 h-12 md:w-14 md:h-14 rounded-xl flex items-center justify-center transition-colors ${notesActive ? 'bg-[#f0ad25] text-white' : 'bg-[#f8f6f0] dark:bg-zinc-800 text-[#f0ad25] group-hover:bg-[#f0ad25] group-hover:text-white'}`}>
