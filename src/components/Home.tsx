...
export const Home: React.FC<HomeProps> = ({ onPlay, onResumeGame, activeGames }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center flex-1 w-full bg-[#f4f2eb] px-4">
      <div className="flex flex-col items-center gap-5 md:gap-8 w-full max-w-[340px] md:max-w-[400px]">
        
        {/* Icona Griglia Centrale */}
        <div className="w-[100px] h-[100px] md:w-[120px] md:h-[120px] bg-[#f0ad25]/20 rounded-3xl flex items-center justify-center mb-2 md:mb-4 transition-all">
          <Grid3X3 size={44} className="text-[#f0ad25] w-12 h-12 md:w-16 md:h-16 transition-all" strokeWidth={1.5} />
        </div>

        <p className="text-gray-800 font-bold text-sm md:text-base">Scegli la difficoltà</p>

        {/* Selettore Difficoltà */}
        <div className="flex items-center justify-between w-full h-[52px] md:h-[60px] bg-white rounded-full px-4 md:px-6 shadow-sm border border-gray-100/50 transition-all">
          <button onClick={handlePrev} className="p-1 text-[#f0ad25] hover:bg-[#f4f2eb] rounded-full transition-all active:scale-75">
          ...
          <div className="overflow-hidden h-full flex items-center justify-center">
            <span key={diffIndex} className={`font-bold text-gray-900 text-[15px] md:text-lg ${slideDir === 'left' ? 'animate-slide-left' : 'animate-slide-right'}`}>
              {difficulties[diffIndex]}
            </span>
          </div>

          <button onClick={handleNext} className="p-1 text-[#f0ad25] hover:bg-[#f4f2eb] rounded-full transition-all active:scale-75">
        ...
        <div className="w-full flex flex-col gap-4 min-h-[120px] md:min-h-[136px] mt-2">
          ...
          {activeGames[difficulties[diffIndex]] && onResumeGame && (
            <button 
              ...
              className="w-full h-[52px] md:h-[60px] flex-none bg-white hover:bg-gray-50 text-[#f0ad25] font-bold rounded-full shadow-sm border-2 border-[#f0ad25] transition-all active:scale-[0.97] text-[15px] md:text-lg animate-pop-in"
            >
              Riprendi Partita
            </button>
          )}
